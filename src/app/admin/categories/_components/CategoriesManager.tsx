...
// Client island for the categories admin page. The modal needs local state
// (open/closed, form fields) and confirm() / fetch interactivity, so it's
// kept as a client component. The page passes initialCategories from a
// server-side prisma query, which removes the useEffect-on-mount fetch
// waterfall and gives the user content on first paint.
//
// After any mutation we refetch via the API route (which the middleware
// gates) rather than calling prisma directly — that keeps the auth check
// in the same place for both initial render (page-level middleware) and
// subsequent fetches.
export default function CategoriesManager({
  ...
  // Slug auto-generation: copy what the user types into name, normalized.
  // Only auto-fills on create — on edit we let the existing slug stand.
